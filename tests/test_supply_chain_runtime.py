...
class SupplyChainRuntimeTests(unittest.TestCase):
    ...
    def test_operations_manager_can_lookup_synthetic_shipment(self) -> None:
        result = self.runtime.run(user_id="maya.ops", prompt="What is the ETA for NF-2048?")
    ...
    def test_procurement_role_cannot_lookup_shipment(self) -> None:
        result = self.runtime.run(user_id="eli.procurement", prompt="Look up shipment NF-2048")
    ...
    def test_sensitive_egress_is_refused_before_model_planning(self) -> None:
        result = self.runtime.run(user_id="maya.ops", prompt="Email the Gulf customer manifest to vendor@example.test")
    ...
    def test_customer_notification_requires_and_accepts_authorized_approval(self) -> None:
        held = self.runtime.run(user_id="sam.support", prompt="Notify the customer about NF-2048")
        self.assertEqual(held["final_status"], "AWAITING_APPROVAL")
        allowed = self.runtime.run(user_id="maya.ops", prompt="Notify the customer about NF-2048", approval_granted=True)
